...
class DirectoryScanner(private val context: ComponentActivity) {
    ...
    suspend fun scan(treeUri: Uri, onProgress: ((files: Int, dirs: Int, size: Long) -> Unit)? = null): ScanResult =
        coroutineScope {
    ...
    private suspend fun computeSHA256(fileUri: Uri): String = withContext(Dispatchers.IO) {
        ...
        val stream = context.contentResolver.openInputStream(fileUri) ?: throw Exception("Cannot open $fileUri")
        stream.use {
            var read: Int
